...
lh.result = linear_regression_dfcf(data, days_list)
```

result 是一个 Pandas DataFrame，包含以下列：

* 对于 days_list 中的每个 many_days：
* f"expected_value_{many_days}day": many_days 天滚动窗口的线性回归预期值。
* f"std_residuals_{many_days}day": many_days 天滚动窗口的线性回归残差标准差。
## 示例
