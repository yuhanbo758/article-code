...
* price (float): 触发价格
* quantity (int): 交易数量
* buy_sell (str): 买卖方向 ('buy' 或 'sell')
* strategy (str): 策略名称
* remark (str): 备注信息
* datetime_str (str): 预设执行时间，格式为 "%Y%m%d %H:%M"
...
* 如果设置了预设执行时间，且当前时间晚于预设时间，则删除该条记录以及对应的 execute_general_trade 表中的记录，并跳过后续处理。
* 根据股票代码从行情数据中获取对应的股票实时行情，如果行情数据为空，则跳过该条记录。
* 根据策略类型进行不同的处理：
* 即时委托：直接下单并删除记录。
* 上破价：如果最新价大于等于触发价格，则下单并删除记录。
* 下破价：如果最新价小于等于触发价格，则下单并删除记录。
* 封单不足-注数量：根据备注信息判断买卖方向和触发数量，如果满足条件则下单并删除记录。
* 回落卖-价涨幅-注回撤：计算价格涨幅和回撤幅度，如果满足条件则以最优五档价格下单并删除记录。
* 反弹买-价跌幅-注反弹：计算价格跌幅和反弹幅度，如果满足条件则以最优五档价格下单并删除记录。
* 价差网格-注价差：调用 process_price_grid 函数处理价差网格策略。
* 振幅网格-注振幅：调用 process_amplitude_grid 函数处理振幅网格策略。
* 其他策略：不做处理，直接跳过。
### 代码示例
