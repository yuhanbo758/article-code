...
函数调用 calculate_stop_profit_loss(target_strategies, profit_multipliers, loss_multipliers, db_path)，无需返回值。db_path 必须显式指定，避免函数误写到开发者个人电脑的默认数据库。
...
lh.calculate_stop_profit_loss(target_strategies, profit_multipliers, loss_multipliers, db_path)
```
