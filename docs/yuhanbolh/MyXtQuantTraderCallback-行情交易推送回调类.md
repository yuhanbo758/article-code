...
* 资金变动推送，目前不生效。
* 参数: asset (XtAsset): 资产信息对象。
* 功能: 将接收到的资金信息保存到数据库的 manage_assets 表中。
* on_stock_trade(self, trade)
* 成交变动推送。
* 参数: trade (XtTrade): 成交信息对象。
* 功能: 将接收到的成交信息保存到数据库的相应策略交易记录表中，例如 execute_fund_grid_trade、execute_fund_basics_technical_trade 等。
