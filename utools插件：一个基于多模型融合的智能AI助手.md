...
1. uTools 插件应用市场安装“余汉波 AI 助手”
1. 在 uTools 中唤起插件（可通过关键词"AI"或设置的自定义关键词）
1. 在输入框中输入问题或指令
1. 按 Enter 发送消息，或使用 Shift+Enter 插入换行
1. 注意，模型增加除了openrouter 需要增加模型路径外（比如 openai/gpt-4.1 中的 openai），其他的都不用（比如 gemini，模型直接增加gemini-2.0-flash）。
### 提示词模板

1. 在输入框中输入@符号可调出提示词搜索框
1. 选择合适的提示词模板后将自动添加为系统消息
1. 可以在设置中配置提示词文件夹，存放自定义提示词模板
### 模型选择

1. 在顶部下拉菜单中选择要使用的 AI 模型
1. 可以在设置中添加自定义模型
1. 设置中可指定默认使用的模型
### 会话管理

1. 点击"新对话"按钮开始新的对话
1. 点击"历史记录"按钮查看和加载之前保存的对话
1. 点击"保存对话"按钮将当前对话保存到指定文件夹
# 简介
