...
主页：[三人聚智-余汉波 - CodeBot](https://www.sanrenjz.com/codebot/)
...
* Hermes Agent 深度集成（接入重构、运行状态可视化、技能共享与兼容分流、输出优化）
* Obsidian 知识库集成（多知识库支持、技能与命令搜索增强）
* 模型管理与对话状态（CLI 集成、对话级状态独立、定时任务执行器绑定、调度边界控制）
* 技能系统增强（对话生成技能、内部上下文隔离）
* 流式响应与事件展示优化（流式链路升级、后台事件回放、消息展示优化）
* 记忆系统增强（分类补充、AI 提取增强、事实同步、存储诊断）
* 定时任务增强（候选通知、意图识别优化）
* OpenAI 兼容模型网关
* 连接状态与启动优化
* 分享与归档
